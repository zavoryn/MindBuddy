...
from mindbuddy.state import add_cost, record_api_error, update_context_usage
...
def _get_retry_limit() -> int:
    try:
        value = int(float(os.environ.get("MINDBUDDY_MAX_RETRIES", DEFAULT_MAX_RETRIES)))
    except ValueError:
    ...
    def next(self, messages: list[dict[str, Any]], on_stream_chunk: Callable[[str], None] | None = None, on_thinking_delta: Callable[[str], None] | None = None, store: Store[AppState] | None = None) -> AgentStep:
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                from mindbuddy.api_retry import classify_error
