...
from mindbuddy.file_review import apply_reviewed_file_change, load_existing_file
...
def _format_mismatch_diagnostic(content: str, search: str) -> str:
    ...
    if best_start >= 0 and best_ratio > 0.3:
        ...
        if best_ratio < 1.0:
            lines.append("")
            lines.append("Hints:")
            search_norm = [_normalize_line(ln) for ln in search_lines]
            content_norm = [_normalize_line(ln) for ln in content_lines[best_start:best_start + window_size]]
    ...
    if len(matches) > 1 and not input_data["replace_all"]:
        ...
        return ToolResult(
            ...
            output=(
                ...
                f"Matches at lines: {', '.join(str(ln) for ln in match_lines)}"
            ),
        )
