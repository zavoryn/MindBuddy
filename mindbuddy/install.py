"""Interactive installer for MindBuddy Python.
...
def _install_launcher_script() -> str | None:
    ...
    if sys.platform == "win32":
        # Windows: Use ~/.mindbuddy/bin with .bat script
        target_bin_dir = MINDBUDDY_DIR / "bin"
        launcher_path = target_bin_dir / "mindbuddy.bat"
        python_exe = sys.executable.replace("/", "\\")
        launcher_script = "\r\n".join([
            "@echo off",
            "REM MindBuddy Python Launcher for Windows",
            f'"{python_exe}" -m mindbuddy.main %*',
            "",
        ])
        launcher_command = "mindbuddy.bat"
    elif sys.platform == "darwin":
        # macOS: Use ~/.local/bin with bash script (also works with zsh)
        target_bin_dir = home / ".local" / "bin"
        launcher_path = target_bin_dir / "mindbuddy-py"
        python_exe = sys.executable
        launcher_script = "\n".join([
            "#!/usr/bin/env bash",
            "# MindBuddy Python Launcher for macOS",
            "# Works with bash, zsh, and other shells",
            "set -euo pipefail",
            f'exec "{python_exe}" -m mindbuddy.main "$@"',
            "",
        ])
        launcher_command = "mindbuddy-py"
    else:
        ...
        launcher_script = "\n".join([
            "#!/usr/bin/env bash",
            "# MindBuddy Python Launcher for Linux",
            "set -euo pipefail",
            f'exec "{python_exe}" -m mindbuddy.main "$@"',
            "",
        ])
    ...
    try:
        ...
        print(f"\n⚠️  无法安装启动器脚本: {e}")
        print("你可以手动创建启动器脚本来调用 mindbuddy。")
        return None
...
def main() -> None:
    ...
    print("=" * 60)
    print("  MindBuddy Python 安装向导")
    print("=" * 60)
    print()
    print(f"配置会写入: {MINDBUDDY_SETTINGS_PATH}")
    ...
    try:
        ...
        print(f"✅ 配置已保存到: {MINDBUDDY_SETTINGS_PATH}")
    except OSError as e:
        print(f"\n❌ 保存配置失败: {e}")
        sys.exit(1)
    ...
    print("=" * 60)
    print()
    print("📁 配置文件:", MINDBUDDY_SETTINGS_PATH)
    if launcher_result:
    ...
    print()
    print("  Windows:")
    print("    mindbuddy.bat               (如果已添加 PATH)")
    print("    python -m mindbuddy.main    (通用方式)")
    print()
    print("  macOS:")
    print("    mindbuddy-py                (如果已添加 PATH)")
    print("    python3 -m mindbuddy.main   (通用方式)")
    print()
    print("  Linux:")
    print("    mindbuddy-py                (如果已添加 PATH)")
    print("    python3 -m mindbuddy.main   (通用方式)")
    print()
    print("感谢使用 MindBuddy Python！🎉")
    print()
