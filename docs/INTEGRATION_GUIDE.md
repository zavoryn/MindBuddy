...
| 文件 | 行数 | 功能 |
|------|------|------|
| `mindbuddy/state.py` | 280 | Store 状态管理 + AppState |
| `mindbuddy/cost_tracker.py` | 280 | 费用追踪 + 使用统计 |
| `mindbuddy/tooling.py` | 扩展 | Tool Protocol + Metadata |
...
```python
from mindbuddy.state import create_app_store, format_app_state_summary
...
# 更新状态
from mindbuddy.state import set_busy, set_idle, update_context_usage
...
```python
from mindbuddy.tooling import Tool, ToolMetadata, ToolCapability
...
```python
# tty_app.py 中已添加：
from mindbuddy.state import AppState, Store, create_app_store
from mindbuddy.cost_tracker import CostTracker
...
编辑 `mindbuddy/cli_commands.py`，添加：
...
编辑 `mindbuddy/agent_loop.py`，在 API 调用后添加：
...
编辑 `mindbuddy/tty_app.py` 的 `on_tool_result` 回调：
...
| 维度 | Claude Code | MindBuddy Python (之前) | MindBuddy Python (现在) |
|------|-------------|----------------------|----------------------|
